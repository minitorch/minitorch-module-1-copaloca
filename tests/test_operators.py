...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    if b is not None:
        # assert lt(sigmoid(a), sigmoid(b)) if lt(a, b) else not lt(sigmoid(a), sigmoid(b))
        assert almost_equal(sigmoid(a), sigmoid(b)) or (
            sigmoid(a) < sigmoid(b) if a < b else sigmoid(a) > sigmoid(b)
        )
